...
public class AllPlayHandler extends BaseThingHandler
    ...
    private void handleZoneMembersCommand(Command command) throws SpeakerException {
        String[] memberNames = command.toString().split(",");
        List<String> memberIds = new ArrayList<>();
        for (String memberName : memberNames) {
            memberIds.add(getHandlerIdByName(memberName.trim()));
        }
    ...
    private String getHandlerIdByName(String remotePlayerName) throws IllegalStateException {
        if (thingRegistry != null) {
            for (Thing thing : thingRegistry.getAll()) {
                if (remotePlayerName.equals(thing.getConfiguration().get(AllPlayBindingConstants.DEVICE_NAME))) {
